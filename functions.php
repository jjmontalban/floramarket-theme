...
/**
 * @snippet Sincronizacion de productos
 * @author   Eurosystem   
 */

// Función que se ejecutará cada 10 minutos
function eliminar_archivos_temp_ftp() {
    // Configuración de conexión FTP
    $ftp_server = '185.177.154.89';
    $ftp_user = 'floramarket';
    $ftp_pass = 'Gb11550.Gb11550.';

    // Conectar al servidor FTP
    $ftp_connection = ftp_connect($ftp_server);
    $login = ftp_login($ftp_connection, $ftp_user, $ftp_pass);

    // Directorio remoto donde se encuentran los archivos
    $directorio_remoto = '/httpdocs/wp-content/webtoffee_import/';

    // Listar archivos en el directorio remoto
    $archivos = ftp_nlist($ftp_connection, $directorio_remoto);

    if ($login && $archivos !== false) {
        // Iterar a través de los archivos y eliminar los que comienzan con "temp_"
        foreach ($archivos as $archivo) {
            if (strpos($archivo, 'temp_') === 0) {
                ftp_delete($ftp_connection, $archivo); // Eliminar el archivo
            }
        }
        ftp_close($ftp_connection); // Cerrar la conexión FTP
    }
}

// Programar la tarea para que se ejecute cada 10 minutos
function programar_eliminar_archivos_temp_ftp() {
    if (!wp_next_scheduled('eliminar_archivos_temp_ftp_event')) {
        wp_schedule_event(time(), 'hourly', 'eliminar_archivos_temp_ftp_event');
    }
}
add_action('wp', 'programar_eliminar_archivos_temp_ftp');
add_action('eliminar_archivos_temp_ftp_event', 'eliminar_archivos_temp_ftp');
